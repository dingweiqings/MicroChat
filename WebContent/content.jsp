...
if(session.getAttribute("name").equals("null")){
	out.println("<script language='javascript'>alert('您还没有登录不能进入本聊天室');parent.location.href='login.jsp';</script>");
}
if(session.getAttribute("name").equals("request.getParameter("+request.getParameter("tempuser")+")")){
...
if(message!=null&&tempuser!=null){
	...
	if(application.getAttribute("message")==null){	//第一个人说话时
		...
		"<strong>"+session.getAttribute("name")+"</strong>"+"</font>"+"<font color='#CC0000'>"+select+"</font>"+"对"+"<font color='green'>"+"["+tempuser+"]"+"</font>"+"说："+"<font color="+color+">"+message);
	}else{
		application.setAttribute("message","<br>"+"<font color='blue'>"+"<strong>"+session.getAttribute("name")+"</strong>"+"</font>"+"<font color='#CC0000'>"+select+"</font>"+"对"+"<font color='green'>"+"["+tempuser+"]"+"</font>"+"说："+"<font color="+color+">"+message+"</font>"+application.getAttribute("message"));
	}
	out.println("<p>"+application.getAttribute("message")+"<p>");
}else{
	if(application.getAttribute("message")==null){
		out.println("欢迎"+"<font color='#cc0000'>"+session.getAttribute("name")+"</font>"+"<font color='green'>"+"走进了网络聊天室"+"</font>");
